...
{
	RGBAPixel color;
	//cout<<""<<fade2.red<<fade2.green<<fade2.blue<<endl;
	/**
	 * @todo Return the correct color here!
	 */
	int d = abs(cenX - x) + abs(cenY - y);
	if(d < r)
	{
		int rFill = fade1.red - floor((d*fade1.red)/r) + floor((d*fade2.red)/r);
		int gFill = fade1.green - floor((d*fade1.green)/r) + floor((d*fade2.green)/r);
		int bFill = fade1.blue - floor((d*fade1.blue)/r) + floor((d*fade2.blue)/r);
		color.red = rFill;
		color.green = gFill;
		color.blue = bFill;
	}
