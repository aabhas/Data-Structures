...
template <template <class> class OrderingStructure>
animation filler::fill( PNG & img, int x, int y,
     ...
     *        it's RIGHT, then the one DOWN from it, then to the LEFTsend the current PNG to the
     *              animation and
    ...
    OrderingStructure<pair <int,int> > structure;
    bool processed[img.width()][img.height()];
    int frameCount = frameFreq;
    int myTol = 0;
    animation output;
    RGBAPixel x2 = *img(x,y);
    int y2 = y;
    ...
    structure.add(make_pair(x,y));
    while(!(structure.isEmpty()))
    {
    	std::pair <int,int> foo;
    	foo = structure.remove();
    	int foo_x = foo.first;
    	int foo_y = foo.second;
    	myTol = pow((img(foo_x,foo_y)->red - x2.red), 2) + pow((img(foo_x,foo_y)->green - x2.green), 2) + pow((img(foo_x,foo_y)->blue - x2.blue), 2);
    	if((!(processed[foo_x][foo_y]))&&(!(myTol > tolerance)))
    	{
    		processed[foo_x][foo_y] = true;
    		*img(foo_x,foo_y) = fillColor(foo_x, foo_y);
    		if(!((foo_x+1) > (img.width()-1))) structure.add(make_pair(foo_x+1,foo_y));
    		if(!((foo_y+1) > (img.height()-1))) structure.add(make_pair(foo_x,foo_y+1));
    		if(!((foo_x-1) < 0)) structure.add(make_pair(foo_x-1,foo_y));
    		if(!((foo_y-1) < 0)) structure.add(make_pair(foo_x,foo_y-1));
   			frameCount--;
   		}
